...
and stores the model during `.fit()`, making subsequent `.predict()` calls fast by using a KV-Cache.

- It is enabled by setting the `fit_mode` parameter of `TabPFNClassifier` and `TabPFNRegressor` to
`fit_with_cache`. However, with this setting classification models will consume approximately 6.1
