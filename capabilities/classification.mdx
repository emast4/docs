...
You can still manually set parameters if you don't use `tuning_config`.

#### Manual Temperature

Setting `softmax_temperature` directly (without `tuning_config`) applies a fixed temperature scaling.

-   `softmax_temperature < 1.0`: Makes predictions _more_ confident (sharper probabilities).
    
-   `softmax_temperature > 1.0`: Makes predictions _less_ confident (softer probabilities).
    

```python
# Manually create a "less confident" model
calibrated_model = TabPFNClassifier(softmax_temperature=1.5)
