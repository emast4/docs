...
TabPFN can also be applied to regression tasks using the [`TabPFNRegressor`](https://github.com/PriorLabs/tabpfn/blob/main/src/tabpfn/regressor.py) class. The regressor is built to make forecasts or predict an output distribution that measures uncertainty.
...
- **Training in seconds:** Generates predictions in a single forward pass using the provided context window.
- **Calibrated uncertainty:** Produces mean, median, or quantile-based predictions for confidence estimation.
