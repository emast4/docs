...
TabPFN can also be applied to regression tasks using the `TabPFNRegressor` class. The regressor is built to make forecasts or predict an output distribution that measures uncertainty.
...
The `TabPFNRegressor.predict()` method is powerful and can return different types of predictions based on the `output_type` parameter.
...
- Balances accuracy vs. runtime with the `max_time` parameter.

```python
from tabpfn_extensions.post-hoc-ensembles.sklearn_interface import AutoTabPFNRegressor
...
<CardGroup cols={3}>
  ...
  <Card title="Interpretability" icon="search" href="/capabilities/interpretability">
    Explain TabPFN predictions with SHAP values and feature selection.
  </Card>
