---
title: "Overview"
description: "One model, infinite predictions."
---

![Screenshot 2025-10-28 at 12.22.26.png](/images/Screenshot2025-10-28at12.22.26.png)

## Welcome to Prior Labs

TabPFN is a **foundation model for tabular data** - built to replace manual model tuning with instant, state-of-the-art predictions.

It’s trained across millions of synthetic datasets to learn the learning process itself. Instead of optimizing weights for every new dataset, TabPFN already encodes the inductive biases, priors, and optimization strategies that traditional models must rediscover each time. When you run it on your data, it performs **zero-shot inference** - producing predictions in seconds that match or exceed the accuracy of tuned ensembles trained for hours.

### Why teams choose TabPFN

<Columns cols={2}>
  <Card title="Accurate predictions in seconds" icon="zap">
    TabPFN-2 delivers benchmark-level performance instantly, achieving tuned-ensemble accuracy in seconds, not hours.
  </Card>
  <Card title="No training required" icon="repeat">
    Skip the training loop. TabPFN performs zero-shot inference with no tuning or gradient descent.
  </Card>
  <Card title="Seamless integration" icon="puzzle">
    Plug into any workflow with the familiar `scikit-learn` interface or through the Prior Labs API for production use.
  </Card>
  <Card title="Robust in the real world" icon="shield-check">
    Handles missing values, outliers, and categorical features natively for stable, reliable performance.
  </Card>
  <Card title="Minimal preprocessing" icon="gear">
    Works directly on raw tabular data - no encoding, scaling, or imputation pipelines required.
  </Card>
  <Card title="Interpretable & trustworthy" icon="search">
    Produces calibrated probabilities and integrates with SHAP for transparent, explainable outcomes.
  </Card>
</Columns>

### Capabilities

TabPFN handles **classification**, **regression**, and **time series forecasting**. Customers use it to power high-impact predictions: from disease diagnosis and risk modeling to energy forecasting and industrial optimization.

<CardGroup cols={3}>
  <Card title="Classification" icon="bullseye" href="/capabilities/classification">
    Predict categorical outcomes with calibrated probabilities and strong zero-shot accuracy.
  </Card>
  <Card title="Regression" icon="puzzle" href="/capabilities/regression">
    Estimate continuous values with uncertainty-aware outputs and minimal preprocessing.
  </Card>
  <Card title="Forecasting" icon="line-chart" href="/capabilities/forecasting">
    Model tabular time series (via TabPFN for forecasting) to predict future values and trends.
  </Card>
</CardGroup>

## Get Started

Ready to try TabPFN? Choose your path:

<CardGroup cols={2}>
  <Card title="Quickstart Guide" icon="rocket" href="/quickstart">
    Get up and running in minutes with step-by-step instructions
  </Card>
  <Card title="Use Cases" icon="lightbulb" href="/use-cases">
    Explore real-world applications and examples
  </Card>
</CardGroup>