...
<Info>
Please note that TabPFN-2.5 is available only for **non-commercial use**, whereas TabPFN-2 is available for any usage.
</Info>
...
Instantiate the hosted [`tabpfn_client.TabPFNClassifier`](https://github.com/PriorLabs/tabpfn-client/blob/main/tabpfn_client/estimator.py) for API-backed predictions:
...
Similarly, the [`tabpfn_client.TabPFNRegressor`](https://github.com/PriorLabs/tabpfn-client/blob/main/tabpfn_client/estimator.py) mirrors the regression interface:
...
In order to access TabPFN 2.5 through the Python SDK you will need a recent version of `tabpfn-client`. For best results we recommend upgrading to the most recent version with, e.g.: 
```bash
pip install -U tabpfn-client
```

If you do this, you'll immediately have access to TabPFN 2.5. Here are some examples:

```python
# By default use TabPFN-2.5
model = TabPFNClassifier(n_estimators=4)

# Specify an exact model checkpoint
model = TabPFNClassifier(n_estimators=4, model_path="v2.5_default")

# Use previous TabPFN-2 model weights
model = TabPFNClassifier(n_estimators=4, model_path="v2_default")
```

We also provide a convenience method for listing the available models:
