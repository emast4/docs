...
See the details of the [available models](models.mdx).
...
- Instantiate the classifier or regressor:
  ...
  regressor = TabPFNClassifier.get_default_for_version(ModelVersion.V2_5)
  ```
...
- Instantiate the classifier or regressor:
  ...
  ```python
  from tapfn_client.estimator import TabPFNClassifier, TabPFNRegressor
...
```python
TabPFNClassifier().list_available_models()
```

### API via REST

The model is selected using the "model_path" in the payload. The default model paths are as follows:

- TabPFN-2.5
  - classification: `tabpfn-v2.5-classifier-v2.5_default.ckpt`
  - regression: `tabpfn-v2.5-regressor-v2.5_default.ckpt`
- TabPFN-2
...
TabPFN (v1) is only available through the OSS package, via the [tag on GitHub](https://github.com/PriorLabs/TabPFN/releases/tag/v1.0.0).
